...
const data = computed(() => {
    ...
    for (const floodMonitoring of floodMonitoringStore.getFloodMonitoringsByZoneId(selectedZoneId.value)) {
        const row = {
            ...
            minimumThreshold: (getAlertSettings(floodMonitoring.hasDeviceMeasurement, "LT").length ? `${getAlertSettings(floodMonitoring.hasDeviceMeasurement, "LT")[0].criteriaValue} mm` + (subscriptionStore.getSubscription(getAlertSettings(floodMonitoring.hasDeviceMeasurement, "LT")[0].hasSubscription).notification.lastNotification ? ` (${formatDate(subscriptionStore.getSubscription(getAlertSettings(floodMonitoring.hasDeviceMeasurement, "LT")[0].hasSubscription).notification.lastNotification)})` : "") : ""),
            maximumThreshold: (getAlertSettings(floodMonitoring.hasDeviceMeasurement, "GT").length ? `${getAlertSettings(floodMonitoring.hasDeviceMeasurement, "GT")[0].criteriaValue} mm` + (subscriptionStore.getSubscription(getAlertSettings(floodMonitoring.hasDeviceMeasurement, "GT")[0].hasSubscription).notification.lastNotification ? ` (${formatDate(subscriptionStore.getSubscription(getAlertSettings(floodMonitoring.hasDeviceMeasurement, "GT")[0].hasSubscription).notification.lastNotification)})` : "") : "")
        };
        rows.push(row);
    }
...
<template>
    <div class="container">
        ...
        <CityZonePicker v-model:selected-city-id="selectedCityId" v-model:selected-zone-id="selectedZoneId" :responsibilities="['WATERCOURSES']" class="mb-4" />
