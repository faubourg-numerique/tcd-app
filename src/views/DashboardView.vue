...
<template>
    <div class="container">
        <LMap style="min-height: 60vh" :zoom="dashboardMapZoom"
            :center="[dashboardMapCenterLatitude, dashboardMapCenterLongitude]" :use-global-leaflet="false">
            <LTileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" layer-type="base"
                name="OpenStreetMap" />
            <LPolygon v-for="zone in zoneStore.zones" :key="zone.id"
                :lat-lngs="zone.location.map((location) => [location[1], location[0]])" />
            <LMarker v-for="wasteContainer in wasteContainerStore.wasteContainers" :key="wasteContainer.id"
                :lat-lng="[wasteContainer.location[1], wasteContainer.location[0]]" :icon="wasteContainerIcon"
                @click="openWasteContainer(wasteContainer.id)" />
            <LMarker v-for="floodMonitoring in floodMonitoringStore.floodMonitorings" :key="floodMonitoring.id"
                :lat-lng="[floodMonitoring.location[1], floodMonitoring.location[0]]" :icon="floodMonitoringIcon"
                @click="openFloodMonitoring(floodMonitoring.id)" />
            <LMarker v-for="streetlight in streetlightStore.streetlights" :key="streetlight.id"
                :lat-lng="[streetlight.location[1], streetlight.location[0]]" :icon="streetlightIcon"
                @click="openStreetlight(streetlight.id)" />
            <LMarker v-for="city in cityStore.cities" :key="city.id" :lat-lng="[city.location[1], city.location[0]]"
                :icon="cityIcon" />
            <LMarker v-for="thermostat in thermostatStore.thermostats" :key="thermostat.id"
                :lat-lng="[thermostat.location[1], thermostat.location[0]]" :icon="thermostatIcon"
                @click="openThermostat(thermostat.id)" />
        </LMap>
    </div>
</template>
